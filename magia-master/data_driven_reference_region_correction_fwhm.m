...
temp_data = spm_read_vols(V);
temp_data(isnan(temp_data)) = 0;
uncorrected_reference_region = logical(temp_data);
...
% Save QC figure as PNG in output directory
qc_outdir = fileparts(V.fname);
qc_figfile = fullfile(qc_outdir, [n '_dc_qc.png']);
saveas(fig, qc_figfile);
