...
% magia_spm_frame_viewer(subject, specs)
...
switch specs.magia.model
    ...
    case 'srtm'
        ...
        % theta3_list = X(:,2)./(X(:,3) + 1); % Yanw; theta3=k2/(BP+1)

        if(specs.magia.cpi)
            half_life = get_half_life(specs.study.tracer);

            %parametric_images = Gunn1997_nifti_mask(modeling_options.lb, modeling_options.ub,modeling_options.theta3_lb,modeling_options.theta3_ub,modeling_options.nbases,half_life,cr,specs.study.frames,pet_file,brainmask,results_dir);
        
            parametric_images = Gunn1997_nifti_mask_(modeling_options.lb, modeling_options.ub,modeling_options.theta3_lb,modeling_options.theta3_ub,modeling_options.nbases,half_life,cr,specs.study.frames,pet_file,brainmask,results_dir,'HarvardOxford-combined-maxprob-thr25-2mm_YCG.nii');
...
if(specs.magia.cpi)
    ...
    if(specs.magia.fwhm_post)
        ...
        smooth_img(normalized_images(1:end),specs.magia.fwhm_post);
    end
end
